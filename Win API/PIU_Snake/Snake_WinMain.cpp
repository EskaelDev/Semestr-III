...
RECT foodRect;
COLORREF foodColor = RGB(250, 0, 0);
...
struct Container { // info o segmetnach weza
	RECT rect;
	RECT safeOldRect;
	COLORREF color;
	DIRECTION direction;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg)
	{
		...
		case WM_PAINT:
			...
			swprintf(buff, TEXT("PUNKTY: %d               ROZMIAR: %d"), score, posContainer.size());

			SetTextColor(hdc, RGB(0, 0, 0));
			TextOut(hdc, 50, 10, buff, wcslen(buff));
